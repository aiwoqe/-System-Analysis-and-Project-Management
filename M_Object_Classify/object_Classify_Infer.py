...
def postprocess(input_image, output, input_width, input_height, img_width, img_height):
    ...
    return input_image, [class_ids[i] for i in indices], [scores[i] for i in indices]
...
def yolo(image, session, model_inputs, input_width, input_height):
    """
    执行对象检测并返回检测后的图像和检测结果字典。

    参数:
        image: 输入图像，支持PIL对象或NumPy数组。
        session: ONNX运行会话。
        model_inputs: 模型输入信息。
        input_width: 模型输入的宽度。
    ...
    返回:
        output_image: 绘制了检测结果的图像。
        detection_info: 检测结果的字典，包括类别和得分。
    """
    ...
    output_image, class_ids, scores = postprocess(result_image, outputs, input_width, input_height, img_width,
                                                  img_height)

    # 构建检测结果的字典
    detection_info = [{'class': classes[class_id], 'score': score} for class_id, score in zip(class_ids, scores)]
...
if __name__ == '__main__':
    ...
    mode = 1
    if mode == 1:
        image_data = cv2.imread("./data/img.png")
        # 使用检测模型对读入的图像进行对象检测
        result_image, detection_info = yolo(image_data, session, model_inputs, input_width, input_height)
